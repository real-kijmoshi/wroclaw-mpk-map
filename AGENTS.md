...
leaves it null instead of denying it. Low floor also does not establish wheelchair
space or boarding access; the bundled roster leaves wheelchair unknown until a
...
- Before store submission: backend on HTTPS with a real hostname,
  ...
  Play Data Safety and Apple privacy labels. Android uses the Leaflet surface;
  iOS uses MapKit, so neither needs a Google Maps SDK key.
